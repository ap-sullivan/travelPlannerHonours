// fetchWiki tries to get the best possible Wikipedia article for a given attraction
...
import {
  isRejectedCategory,
  isCoordMismatch,
} from "../../utils/wikiValidation";

const WIKI_HEADERS = {
  "User-Agent":
    "PlanMyScotTrip/1.0 (https://github.com/ap-sullivan/travelPlannerHonours",
  Accept: "application/json",
};

export async function fetchWiki(attraction) {
...
async function fetchWikiByTitle(query, lat, lon) {
  ...
  if (
    isCoordMismatch({ lat: wikiCoords.lat, lon: wikiCoords.lon }, { lat, lon })
  ) {
    return null;
  }
